...
if __name__ == "__main__":
    infer_path = Path('/media/hugobaudchon/4 TB/XPrize/infer/20240521_zf2100ha_highres_m3m_TILES/20240521_zf2100ha_highres_m3m_rgb')
    ...
    checkpoint = '/media/hugobaudchon/4 TB/XPrize/trainings_2/contrastive_resnet50_768_1024_96_genus_1719315836/checkpoint_7.pth'
    ...
    with torch.no_grad():
        test_loader = torch.utils.data.DataLoader(dataset, batch_size=8, shuffle=False, num_workers=4, collate_fn=contrastive_collate_fn)
        infer_loader = torch.utils.data.DataLoader(infer_dataset, batch_size=8, shuffle=False, num_workers=4, collate_fn=contrastive_infer_collate_fn)

        test_labels, test_labels_ids, test_families, test_families_ids, test_embeddings, test_predicted_families = infer_model_with_labels(
            model,
            test_loader,
            device,
        ...
        infer_embeddings, infer_predicted_families = infer_model_without_labels(
            model,
            infer_loader,
            device,
            use_mixed_precision=False,
